#!/usr/bin/env python3
"""PRACTICE runner — ZETA HYBRID with 2-min expiry."""
import sys, os, time, json, importlib

AMOUNT = 5
BASE_DIR = "/root/iqoption-bot/experiments"
RESULTS_DIR = os.path.join(BASE_DIR, "results")
LOG_FILE = os.path.join(RESULTS_DIR, "zeta_2min_live.log")
TRADES_FILE = os.path.join(RESULTS_DIR, "zeta_2min_live_trades.json")
...
for asset in top:
    ...
    # 2-minute expiry
    try:
        ok, tid = api.buy(AMOUNT, asset, direction, 2)
        if not ok:
            log(f"  {asset} {direction} FAIL: {tid}")
            continue
    except: continue

    time.sleep(125)  # 2-min wait
    ...
    trade = {"time": time.strftime('%Y-%m-%d %H:%M:%S'), "asset": asset,
             ...
             "profit": profit, "win": win, "expiry": "2min"}
    trades.append(trade)
