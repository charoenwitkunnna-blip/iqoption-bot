#!/usr/bin/env python3
"""PRACTICE runner — ZETA 5M (5-min candles, 5-min expiry)."""
import sys, os, time, json, importlib

AMOUNT = 5
BASE_DIR = "/root/iqoption-bot/experiments"
RESULTS_DIR = os.path.join(BASE_DIR, "results")
LOG_FILE = os.path.join(RESULTS_DIR, "zeta_5m_live.log")
TRADES_FILE = os.path.join(RESULTS_DIR, "zeta_5m_live_trades.json")

sys.path.insert(0, BASE_DIR)
from config_practice import IQ_OPTION_EMAIL, IQ_OPTION_PASSWORD
from iqoptionapi.stable_api import IQ_Option

strat = importlib.import_module("new_algos.zeta_5m.strategy")
...
for asset in top:
    ...
    trade = {"time": time.strftime('%Y-%m-%d %H:%M:%S'), "asset": asset,
             ...
             "profit": profit, "win": win, "expiry": "5min"}
    trades.append(trade)
