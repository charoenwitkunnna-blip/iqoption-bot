"""
THETA PRO — Ultra-Selective RSI-2
==================================
RSI(2) < 8 / > 92 in strong trends only.
...
def analyze(api, asset, candles, htf_candles=None):
    ...
    # CALL: Extreme oversold in strong uptrend
    if rsi2 < 8 and trend_up and mom_up and cur_adx >= 22:
        confidence = 70
        if rsi2 < 3: confidence += 10  # Ultra extreme
        return "call", confidence

    # PUT: Extreme overbought in strong downtrend
    if rsi2 > 92 and trend_down and mom_down and cur_adx >= 22:
        confidence = 70
        if rsi2 > 97: confidence += 10
