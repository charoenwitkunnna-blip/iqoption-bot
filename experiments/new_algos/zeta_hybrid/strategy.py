"""
ZETA HYBRID — RSI-2 Extremes + Trend Quality Filters
======================================================
RSI-2 catches the extremes. Trend filters block the bad ones.
The version that hit 78% on REAL.
"""
NAME = "zeta_hybrid"
...
def analyze(api, asset, candles, htf_candles=None):
    ...
    if rsi2 < 15 and trend_up and momentum_up:
        # Scalp: RSI must be recovering (turning up)
        rsi2_prev = rsi(closes[:-1], 2) if len(closes) > 26 else None
        if rsi2_prev is not None and rsi2 <= rsi2_prev:
            return None, 0
        confidence = 55
        if rsi2 < 5: confidence += 15
        if cur_adx > 25: confidence += 10
        if confidence < 60: return None, 0
