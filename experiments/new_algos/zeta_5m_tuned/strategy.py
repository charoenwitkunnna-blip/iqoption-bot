"""
ZETA 5M — Tuned for 5-Minute Candles
======================================
RSI(2) thresholds widened for 5-min timeframe.
5-min candles have fewer extremes, so thresholds are relaxed.
"""
NAME = "zeta_5m_tuned"
...
def analyze(api, asset, candles, htf_candles=None):
    ...
    # Wider thresholds for 5-min
    if rsi2 < 25 and trend_up and mom_up:
        conf = 55
        if rsi2 < 15: conf += 10
        return "call", conf

    if rsi2 > 75 and trend_down and mom_down:
        conf = 55
        if rsi2 > 85: conf += 10
        return "put", conf
