"""
ZETA SMART — Data-Driven Zeta
==============================
- Only trade during best hours (09-11 UTC+7)
- Only CALLS (52% WR vs 44% PUTS in data)
- Higher ADX requirement during off-peak
"""
NAME = "zeta_smart"
...
def analyze(api, asset, candles, htf_candles=None):
    ...
    # Hour filter: only optimal hours (GMT+7 = ICT)
    hour = int(_time.strftime('%H'))
    peak = (9 <= hour <= 10)  # Best hours from data
    offpeak = (11 <= hour <= 15)  # Decent hours
    ...
    # ADX threshold varies by hour
    min_adx = 18 if peak else (20 if offpeak else 25)
    if cur_adx < min_adx: return None,0
    ...
    # CALL bias: more aggressive thresholds for calls, tighter for puts
    if rsi2 < 15 and trend_up and mom_up:
        return "call", 65

    # PUT only during peak hours with strong confirmation
    if peak and rsi2 > 85 and trend_down and mom_down and cur_adx >= 25:
        return "put", 60
