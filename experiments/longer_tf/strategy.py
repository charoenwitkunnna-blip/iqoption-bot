#!/usr/bin/env python3
"""Experiment: 3-minute expiry with 5-min analysis candles.
Longer timeframe reduces noise and improves TA accuracy."""
import sys, os, time, json, importlib
import pandas as pd
import pandas_ta as ta
import numpy as np

STRAT_NAME = "longer_tf"
AMOUNT = 8
...
trades = []
trades_file = f"{BASE_DIR}/results/longer_tf_trades.json"
...
for asset in assets:
    try:
        ...
        if direction in ["call", "put"] and conf >= 50:
            amount = min(AMOUNT, max(3, int(balance * 0.03)))
            ok, tid = api.buy(amount, asset, direction, 3)  # 3-minute expiry!
            log(f"SIG:{asset} {direction.upper()} {amt}(c={conf}/3min) ok={ok}")
            if ok:
                time.sleep(185)  # 3 min + extra
                ...
                trades.append({"time":time.strftime("%Y-%m-%d %H:%M:%S"),"asset":asset,
                              "direction":direction,"amount":amount,"conf":conf,
                              "profit":profit,"win":win,"expiry":"3min"})
