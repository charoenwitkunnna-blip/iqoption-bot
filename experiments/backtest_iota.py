#!/usr/bin/env python3
"""Backtest iota_ensemble vs top performers."""
...
iota = importlib.import_module("new_algos.iota_ensemble.strategy")
omega = importlib.import_module("new_algos.omega_momentum.strategy")
theta = importlib.import_module("new_algos.theta_rsi2.strategy")

strategies = {"iota": iota, "omega": omega, "theta_rsi2": theta}
...
print(f"{'Strategy':<16s} {'W':>4s} {'L':>4s} {'Sig':>4s} {'WR':>6s}  Verdict")
print("-" * 50)
for n, r in sorted(results.items(), key=lambda x: (x[1]['w']/(x[1]['w']+x[1]['l'])) if (x[1]['w']+x[1]['l'])>0 else 0, reverse=True):
    ...
    v="STRONG" if t>0 and r['w']/t>=0.58 else ("GOOD" if t>0 and r['w']/t>=0.54 else "OK" if t>0 and r['w']/t>=0.50 else "WEAK")
    print(f"{n:<16s} {r['w']:>4d} {r['l']:>4d} {t:>4d} {wr:>6s}  {v}")
