#!/usr/bin/env python3
"""V2 cycle on EURJPY-OTC only. 82% backtest win rate."""
import sys, os, time, json, importlib

STRAT_NAME = "v2"
ASSETS_ONLY = ["EURJPY-OTC"]
AMOUNT = 8
BASE_DIR = "/root/iqoption-bot/experiments"
RESULTS_FILE = f"{BASE_DIR}/results/v2_eurjpy_trades.json"
log_file = f"{BASE_DIR}/results/v2_eurjpy_stable.log"
...
try:
    ...
    for asset in ASSETS_ONLY:
        try:
            ...
            result = v2.analyze(api, asset, candles)
    ...
    log(f"CYCLE: {stats['total']}t {stats['wins']}w/{losses}l {wr:.0f}% pnl={stats['pnl']:.1f}")
except Exception as e:
