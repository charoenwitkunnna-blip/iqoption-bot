#!/usr/bin/env python3
"""REAL balance — ZETA HYBRID with 3-min expiry."""
import sys, os, time, json, importlib

AMOUNT = 30
STOP_LOSS = -90  # 3 losing trades
BASE_DIR = "/root/iqoption-bot/experiments"
RESULTS_DIR = os.path.join(BASE_DIR, "results")
LOG_FILE = os.path.join(RESULTS_DIR, "zeta3_real.log")
TRADES_FILE = os.path.join(RESULTS_DIR, "zeta3_real_trades.json")
...
api.connect(); api.change_balance("REAL"); time.sleep(2)
balance = api.get_balance()
log(f"START: bal={balance:.1f}")
...
for asset in top:
    ...
    if total_pnl <= STOP_LOSS:
        log(f"STOP LOSS HIT: pnl={total_pnl:.1f}. Stopping.")
        break
    if balance < AMOUNT:
    ...
    try:
        ok, tid = api.buy(AMOUNT, asset, direction, 3)
        if not ok:
...
log(f"DONE: {t}t {w}w/{t-w}l {f'{w/t*100:.0f}%' if t>0 else '0%'} pnl={pnl:+.1f} bal={balance:.1f}")
