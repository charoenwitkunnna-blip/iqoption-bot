#!/usr/bin/env python3
"""PRACTICE runner — any strategy. Args: strategy_name"""
import sys, os, time, json, importlib

STRAT_NAME = sys.argv[1] if len(sys.argv) > 1 else "gamma_breakout"
AMOUNT = 5
BASE_DIR = "/root/iqoption-bot/experiments"
RESULTS_DIR = os.path.join(BASE_DIR, "results")
LOG_FILE = os.path.join(RESULTS_DIR, f"{STRAT_NAME}_live.log")
TRADES_FILE = os.path.join(RESULTS_DIR, f"{STRAT_NAME}_live_trades.json")

sys.path.insert(0, BASE_DIR)
from config_practice import IQ_OPTION_EMAIL, IQ_OPTION_PASSWORD
from iqoptionapi.stable_api import IQ_Option

strat = importlib.import_module(f"new_algos.{STRAT_NAME}.strategy")
...
paying = {}
for asset in list(all_pairs.keys())[:60]:
    try:
...
for asset in top:
    ...
    # Get 5-min HTF candles for strategies that need them
    htf_candles = None
    try:
        htf_candles = api.get_candles(asset, 300, 10, time.time())
    except: pass

    try: direction, confidence = strat.analyze(api, asset, candles, htf_candles)
    except: continue
    ...
    try:
        ...
        if isinstance(result, (list, tuple)):
            win = bool(result[0])  # Extract first element — the actual win bool
        else:
            win = bool(result)
    except:
        win = False
